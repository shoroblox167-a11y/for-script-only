...
-- Persistent transfer state. Saved UUIDs are intentionally retained even
...
function transferLoadPersistentState()
    ...
    local decodedOK, decoded = pcall(function()
        return TransferHttpService:JSONDecode(raw)
    end)
    ...
    if decoded.enabled ~= nil then
        PersistentTransferConfig.enabled = decoded.enabled == true
    end
    ...
    if decoded.autoAssignTeamsEnabled ~= nil then
        PersistentTransferConfig.autoAssignTeamsEnabled = decoded.autoAssignTeamsEnabled == true
    end
...
function transferSavePersistentState()
    if not (writefile and TransferHttpService) then
        return false
    end
    ...
    local ok, json = pcall(function()
        return TransferHttpService:JSONEncode(payload)
    end)
    if not ok then
        return false
    end
...
function isEggTool(tool)
    ...
    -- Matches the supplied V52 GetEggUsingName logic:
    -- only reject a tool when b exists and is not "c".
    return (b == nil or b == "c") and h ~= nil
end
...
function getNightEggTool()
    ...
    if character then
        ...
        if isEggTool(equipped)
            and equipped:GetAttribute("h") == CONFIG.DEFAULT_EGG
        then
            return equipped
        end
    end
    ...
    if backpack then
        for _, tool in ipairs(backpack:GetChildren()) do
            if isEggTool(tool)
                and tool:GetAttribute("h") == CONFIG.DEFAULT_EGG
            then
                return tool
            end
        end
...
function isToolHeld(tool)
    ...
    for _, child in ipairs(character:GetChildren()) do
        if child:IsA("Tool") then
            if child == tool or string.find(child.Name, tool.Name, 1, true) then
                return true
            end
        end
    end
...
function getTeamSlotCapacity(data)
    -- V52 reads the live ActivePetUI capacity.
    -- Example title: "Active Pets: 0/8"
    local ok, liveMax = pcall(function()
        ...
        local header = holder and holder:FindFirstChild("Header")
        local title = header and header:FindFirstChild("Title")

        if title and type(title.Text) == "string" then
            local value = tonumber(string.match(title.Text, "/(%d+)"))
    ...
    if ok and liveMax then
        return math.min(CONFIG.TEAM_SLOTS, liveMax)
    end

    -- Fallback to the data value only when the live UI is unavailable.
...
function refreshAutoAssignedTeams()
    ...
    if type(State.reductionTeam) ~= "table" then
        State.reductionTeam = {}
    end
    if type(State.koiTeam) ~= "table" then
        State.koiTeam = {}
    end
    ...
    if State.autoAssignTeamsEnabled then
        ...
        local function append(team, allowed)
            ...
            local candidates = collectUUIDsByPetNames(inventory, allowed)
...
-- V29: dedicated UUID refresh so replacing a pet with another UUID is
-- picked up automatically instead of waiting for a hatch/trade action.
...
function placeNightEggsToMaxInternal()
    ...
    while true do
        ...
        if not isToolHeld(tool) then
            ...
            if not equipTool(tool) then
                State.lastStatus = "⏳ Retrying Night Egg equip..."
                task.wait(0.2)
                continue
            end
        end
        ...
        if not isToolHeld(tool) then
            State.lastStatus = "🔴 Night Egg tool is not held."
            task.wait(0.1)
            continue
        end
        ...
        -- Do not count a request as a placed egg until that happens.
...
-- Rapid Gift yields completely while egg placement owns the tool flow.
function placeNightEggsToMax()
    ...
    State.lastStatus = "🥚 Pausing Rapid Gift • placing Night Eggs..."

    -- Let a currently-running gift pass see the mutex and finish.
    local deadline = os.clock() + 1.5
    while State.autoGiftBusy
        and not State.shuttingDown
    ...
    local ok, result = pcall(placeNightEggsToMaxInternal)

    -- Always release the mutex, including when placement throws.
...
function isToolHeldV52Style(tool)
    if not tool or typeof(tool) ~= "Instance" then
        return false
    end
    ...
    for _, child in ipairs(character:GetChildren()) do
        if child:IsA("Tool") then
            if child == tool or string.find(child.Name, toolName, 1, true) then
                return true
            end
        end
    end
...
function isPetUUIDInLiveInventory(uuid)
    if not uuid then
        return false
    end
...
function pruneCompletedGiftLocks()
    -- Kept for compatibility with the existing state table.
    local inventory = getPetInventory(getData())
...
function collectNightEggPetEntries()
    ...
    for _, uuid in ipairs(State.reductionTeam or {}) do
        teamUUIDs[tostring(uuid)] = true
    end

    for _, uuid in ipairs(State.koiTeam or {}) do
        teamUUIDs[tostring(uuid)] = true
    end
    ...
    for uuid, entry in pairs(inventory) do
        ...
        if petData
            and CONFIG.NIGHT_EGG_PETS[petType]
            and not teamUUIDs[normalizedUUID]
        then
            local tool = getToolByPetUUID(normalizedUUID)
            if tool then
                table.insert(result, {
                    uuid = normalizedUUID,
                    tool = tool,
...
function fastGiftAllNightEggPets()
    if State.autoGiftBusy or State.tradeBusy or State.shuttingDown then
        return
    end

    if State.placingEggs then
        return
    end
    ...
    local ok, err = pcall(function()
        -- One pass/rotation sends each distinct UUID once.
        -- When every eligible UUID has been processed, roundSeen is cleared
        -- and the inventory is scanned again from the beginning.
        local roundSeen = {}
        local sentThisRun = 0
        ...
        do
            ...
            for _, pet in ipairs(pets) do
                ...
                if not roundSeen[uuid] then
                    table.insert(candidates, pet)
                end
            end

            if #candidates == 0 then
                -- Completed the current rotation.
                -- Start again from the first UUID that is still present.
                roundSeen = {}
                task.wait()
                continue
            end
            ...
            for _, pet in ipairs(candidates) do
                ...
                if State.placingEggs or not isToolHeldV52Style(tool) then
                    continue
                end
                ...
                local sendOK, sendErr = pcall(function()
                    ...
                    -- Same GivePet path used by the V52-derived V25/V26 gift code.
                ...
                -- Mark only after GivePet returns successfully.
                -- This prevents the same UUID from being fired again
                -- immediately while allowing the next distinct UUID through.
                roundSeen[uuid] = true
                sentAny = true
                sentThisRun += 1
        ...
        if sentThisRun > 0 then
            State.lastStatus =
                "✅ Rapid Gift cycling • sent "
                .. tostring(sentThisRun)
                .. " request"
                .. (sentThisRun == 1 and "" or "s")
                .. "."
        end
    end)
...
function handleArimabnsTrade()
    ...
    -- Do not let a trade ticket collide with a gift request that is currently
    -- holding a pet tool. Finish the single-UUID gift handoff first.
    local giftDeadline = os.clock() + 3
    while State.autoGiftBusy and os.clock() < giftDeadline do
        task.wait(0.05)
    end

    -- Refresh before taking the restore snapshot so team changes are current.
    pcall(refreshAutoAssignedTeams)
    ...
    -- V29: refresh immediately before collecting trade UUIDs.
    pcall(refreshAutoAssignedTeams)
    ...
    -- Re-read the teams before restoring. A pet may have been replaced
    -- while the trade was open and therefore have a different UUID.
    pcall(refreshAutoAssignedTeams)

    local restoreTeam = savedTeam
    if savedTeamName == "Reduction" and type(State.reductionTeam) == "table" and #State.reductionTeam > 0 then
        restoreTeam = State.reductionTeam
    elseif savedTeamName == "Koi" and type(State.koiTeam) == "table" and #State.koiTeam > 0 then
        restoreTeam = State.koiTeam
    end

    if savedTeamName and type(restoreTeam) == "table" and #restoreTeam > 0 then
        pcall(function()
            equipGardenTeam(restoreTeam, savedTeamName)
        end)
    end
...
local staleV22UI = uiParent:FindFirstChild("FableTransferV22")
if staleV22UI then
...
if State.enabled then
    ...
    State.lastStatus = "Reconnected • preparing hatch team..."
else
    ...
    task.spawn(function()
        ...
        do
            ...
            if data then
                ...
                if success then
                    local targetTeam = #ready > 0 and State.koiTeam or State.reductionTeam
                    if isRequestedTeamActuallyEquipped(targetTeam) then
                        break
                    end
                end
            end
...
Varz.StartHatchingSystem = function()
    ...
    -- Start the continuous hatch system immediately.
    -- Team preparation is deferred; the main cycle also verifies/retries
...
-- Persistent Auto Hatch must use the same start/preflight path as the UI toggle.
if State.enabled then
    task.defer(function()
...
function updateCompactSelectedRows(rows, titleLabel, team, fallbackBuilder, data)
    ...
    titleLabel.Text = string.format("LIVE SELECTED %d/%d", shown, getTeamSlotCapacity(data))
...
Threads.main = task.spawn(function()
    while not State.shuttingDown do
        ...
        if State.resumeAt > 0 then
            ...
            State.resumeAt = 0
            State.lastStatus = "Transfer resumed."
        end
        ...
        -- =========================================================
        -- PHASE 1: Inspect the live garden.
        ...
        -- =========================================================
        -- PHASE 2: V52 Reduction team is only needed while eggs are
        -- not ready. There is no MAX requirement here.
        -- =========================================================
        if #readyNightEggs == 0 then
            ...
            if reductionOK then
                ...
                State.lastStatus = "⚠️ Reduction team unavailable • waiting"
            end

            -- Wait for ANY ready Night Egg, regardless of total farm count.
            while State.enabled
                and not State.tradeBusy
                and not State.shuttingDown
        ...
        -- =========================================================
        -- PHASE 3: Koi/Ruby team + hatch every currently ready Night Egg.
        -- =========================================================
        readyNightEggs = getReadyNightEggs()
